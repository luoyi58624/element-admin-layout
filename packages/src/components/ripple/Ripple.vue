...
import { isEmpty } from 'element-admin-layout-utils'
...
function removeRipper() {
	...
	let ripper = unref(rippers.value[index - 1])
	const delay = Math.max(300 - (Date.now() - ripper.key), 0)
	setTimeout(() => {
		const removeRipper = rippers.value.find(item => item.key === ripper.key)
		if (!isEmpty(removeRipper)) {
			removeRipper!.addHideClass = true
		}
		setTimeout(() => {
			rippers.value.pop()
		}, 250)
